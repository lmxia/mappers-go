...
import (
	...
	"github.com/kubeedge/mappers-go/mappers/digitalbow/configmap"
	"github.com/kubeedge/mappers-go/mappers/digitalbow/driver"
...
func setVisitor(visitorConfig *configmap.DigitalbowVisitorConfig, twin *common.Twin, client *driver.DigitalbowClient) {
...
func onMessage(client mqtt.Client, message mqtt.Message) {
	...
	for twinName, twinValue := range delta.Delta {
		...
		var visitorConfig configmap.DigitalbowVisitorConfig
...
// initDigitalbow initialize Digitalbow client.
func initDigitalbow(protocolConfig configmap.DigitalbowProtocolCommonConfig) (client *driver.DigitalbowClient, err error) {
	// TODO: pass the configuration to the client.
	config := driver.DigitalbowConfig{}
	client, _ = driver.NewClient(config)
	return client, nil
}

// initTwin initialize the timer to get twin value.
func initTwin(dev *globals.DigitalbowDev) {
	for i := 0; i < len(dev.Instance.Twins); i++ {
		var visitorConfig configmap.DigitalbowVisitorConfig
		if err := json.Unmarshal([]byte(dev.Instance.Twins[i].PVisitor.VisitorConfig), &visitorConfig); err != nil {
			klog.Errorf("Unmarshal VisitorConfig error: %v", err)
...
func initData(dev *globals.DigitalbowDev) {
...
func initGetStatus(dev *globals.DigitalbowDev) {
...
func start(dev *globals.DigitalbowDev) {
	var protocolCommConfig configmap.DigitalbowProtocolCommonConfig
	...
	var protocolConfig configmap.DigitalbowProtocolConfig
	...
	client, err := initDigitalbow(protocolCommConfig)
	if err != nil {
